...
\begin{enumerate}
    \itemsep0em
    \item We describe the homogenous and isotropic universe that we see today, encapsulated by the FLRW metric.
    This is our "stage" where things happen.
    \item We use the Inflationary Model in cosmology to hypothesize the existence of a scalar field inflaton, denoted $/phi$ 
    that should coincide with the universe that we see today. 
    \item We take note of the fact that during the early universe, we had quantum fluctuations that 
    both the background metric and the inflaton field. 
    \item To analyze better this perturbations, we use the scalar-vector-tensor decomposition. We only take 
    the scalar and tensor components since they are the only ones relevant in the study. 
    We need to keep track of the scalar perturbations, encapsulated in $\zeta$; 
    note that this already takes into account the scalar perturbations of from the quantum fluctuations, 
    and it should clearly take into account the perturbations from the scalar field. Finally, we take note that 
    the we must also keep an eye on the tensor perturbations, denoted as $\gamma$ -- this is related to the 
    gravitational waves brought forth by the quantum fluctuations.
    \item We slot this into our background metric, essentially "perturbing" our metric. 
    \item We take the perturbed metric $g_{\mu \nu}$ then apply the TMM disformal transformation. 
    \item This gives me a new metric denoted as $\hat{g_{\mu \nu}}$. 
    \item Essentially we want to at the perturbations from two different perspectives or "frames". 
    First, we look at what the actual "background" sees($g_{\mu \nu}$) then compare it what the 
    TMM frame sees ($\hat{g_{\mu \nu}}$). Are these perturbations invariant or not when looked at from different perspectives?
\end{enumerate}
