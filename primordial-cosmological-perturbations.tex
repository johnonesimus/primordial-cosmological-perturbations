...
\title{\boldmath Primordial Cosmological Perturbations under the Invertible Higher-Derivative Disformal Transformation}
...
The SVT decomposed perturbed metric with scalar and tensor perturbations is given by
\begin{equation}
    d s ^ { 2 } = - ( 1 + 2 \xi ) d t ^ { 2 } + 2 \psi _ { ; i } d t d x ^ { i } + a ^ { 2 } ( t ) \left[ ( 1 + 2 \zeta ) \delta _ { i j } + \gamma _ { i j } + 2 E _ { ; i j } \right] d x ^ { i } d x ^ { j }
\end{equation}

This may be written in ADM formalism as 
